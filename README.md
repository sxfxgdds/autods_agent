...
其中 `tools/data_tools.py` 提供 `profile_dataset` 与 `audit_dataset`；`tools/modeling_tools.py` 提供 `train_baseline_models`；`agents.py` 的 `build_agents()` 创建 5 个 `AssistantAgent` 与 `RoundRobinGroupChat`；`run.py` 为异步入口并通过 `Console` 打印 `team.run_stream` 过程。
