...
def _read_csv_checked(
    ...
    # 读取 CSV
    df, error_msg = safe_read_csv(csv_path)
    if error_msg:
        return None, f"错误：{error_msg}"
...
async def profile_dataset(csv_path: str, target_col: str) -> str:
    ...
    Returns:
        JSON 字符串，包含数据集概况或错误信息
    """
    logger.info(f"开始分析数据集概况: {csv_path}")
    ...
    try:
        ...
        top_missing_list = [
            {"column": str(idx), "missing_rate": float(val)}
            for idx, val in top_missing.items()
        ]
        ...
        logger.info(f"数据集概况分析完成: {n_rows} 行, {n_cols} 列")
        return build_success_response(payload)

    except Exception as e:  # noqa: BLE001
        logger.error(f"分析数据集概况时出错: {e}")
        return build_error_response(f"分析数据集概况时出错: {type(e).__name__}: {e}")
...
async def audit_dataset(csv_path: str, target_col: str) -> str:
    ...
    Returns:
        JSON 字符串，包含质量问题和建议
    """
    logger.info(f"开始数据质量审查: {csv_path}")
    ...
    try:
        ...
        if len(cols_with_na) > 0:
            ...
            suggestions.append(
                "对数值列可考虑中位数/均值填补；类别列可用众数或单独缺失类别。"
            )
        else:
        ...
        logger.info(f"数据质量审查完成: 发现 {len(issues)} 个问题")
        return build_success_response(payload)

    except Exception as e:  # noqa: BLE001
        logger.error(f"数据质量审查时出错: {e}")
        return build_error_response(f"数据质量审查时出错: {type(e).__name__}: {e}")
